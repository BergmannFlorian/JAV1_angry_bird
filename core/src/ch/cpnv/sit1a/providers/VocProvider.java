package ch.cpnv.sit1a.providers;
...
public class VocProvider {
    ...
    private VocProvider()
    {
        ...
        language = new Language("en", "Anglais");
        languages.add(language);
        language = new Language("es", "Espagnol");
        languages.add(language);
    ...
    public static VocProvider getInstance(){
        if(single_instance == null)single_instance = new VocProvider();
        return single_instance;
    }

    public Vocabulary pickAVoc(){
        return vocs.get(MathUtils.random(0, vocs.size()-1));
    }
